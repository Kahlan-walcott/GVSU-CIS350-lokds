...
BASE_IMG_PATH = 'data/images/'  # Base path for image files
...
class Avatar:
    def __init__(self, maingame, avatar_type, position, size):
        ...
        self.gesture = ''  # Current action of the player
        self.distance = (-3, -3)  # Offset for the animation rendering
    ...
    def set_gesture(self, gesture):
        if gesture != self.gesture:  # If the new action is different from the current action
            ...
            self.animation = self.maingame.resources[self.avatar_type + '/' + self.gesture].duplicate()  # Copy the animation for the new action
    ...
    def update_avatar(self, tilemap, movement=(0, 0)):
        ...
        self.animation.advance()  # Update the current animation frame
